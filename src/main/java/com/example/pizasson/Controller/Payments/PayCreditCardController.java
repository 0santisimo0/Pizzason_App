 ...
 * the Pay Credit Card Controller menu.
 *
...
public class PayCreditCardController {
    ...
    public TextField textFieldNumberCreditCard, textFieldActualDate, textFieldCCV;
     ...
     */
    public PayCreditCardController() {
    ...
    public void toDoIfCondition(ActionEvent actionEvent) throws IOException {
        String text = textFieldCCV.getText();
        if (text.length() < 4 && !RegularExpression.getRegularExpression().validateNumbers(text)) {
            try {
        ...
        else { message.setText("Only 3 numbers"); }
    }
